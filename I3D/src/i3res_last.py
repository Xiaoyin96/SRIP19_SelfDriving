...
class I3ResNet(torch.nn.Module):
    ...
    def forward(self, x):
        x = self.conv1(x) # input size: 4 x 3 x 32 x 224 x 224, output: 4 x 64 x 32 x 112 x 112
        x = self.bn1(x) 
        x = self.relu(x)
        x = self.maxpool(x) # output: 4 x 64 x 32 x 56 x 56

        x = self.layer1(x) # output: 4 x 256 x 32 x 56 x 56
        # print('layer1:', x.shape)
        x = self.maxpool2(x) # output: 4 x 256 x 16 x 56 x 56
        # print('maxpool2:', x.shape)
        x = self.layer2(x) # output: 4 x 512 x 16 x 28 x 28
        # print('layer2:', x.shape)
        x = self.layer3(x) # output: 4 x 1024 x 16 x 14 x 14 --> option 1: extract feature map
        # print('layer3:', x.shape)
        
        x = x.transpose(1,2) # bs x 16 x 1024 x 14 x 14
        x = x[:,-1,:,:,:] # bs x 1024 x 14 x 14
        x = self.layer4_2d(x)  # bs x 2048 x 7 x 7
        x = self.avgpool_2d(x)
        x = x.view(x.size()[0], -1)
        x = self.fc_2d(x) # 1 x 4
        # x = self.drop(x)
        return x

        # if self.conv_class:
        #     x = self.avgpool(x)
        #     r1 = self.classifier(x) # output: 4 x 4 x 1 x 1 x 1
        #     r1 = r1.squeeze(3)
        #     r1 = r1.squeeze(3) # squeeze: 4 x 4 x 1
        #     r1 = r1.mean(2) # 4 x 4 (batch size x class_nb)
        #     if self.side_task:

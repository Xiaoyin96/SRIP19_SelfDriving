...
from .predictor_ns import build_predictor
...
class GeneralizedModel(nn.Module):
    ...
    def forward(self, imgs, proposals, targets=None):
        """
        Arguments:
        ...
        """
        glob_features = self.backbone(imgs) #1,1024,4,40,23
        glob_features = glob_features[:,:,-1,:,:]
        glob_features.view(1,1024,40,23)
        # roi_features, result, _ = self.roi_heads([glob_features], [proposals], [proposals]) # should be list
        # xx = {}
        # xx['glob_feature'] = features # 1 x 1024 x 14 x 14
        # xx['roi_features'] = x # num of bbox x 2048 x 7 x 7
        roi_features = None
        pred = self.predictor(glob_features,roi_features) # torch.size([4])
        return pred

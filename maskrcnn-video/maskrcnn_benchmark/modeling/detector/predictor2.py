...
class Predictor(nn.Module):
    ...
    def forward(self, x):
        ...
        obj = x['roi_features'] # (N, 2048, 4, 4)

        glob2 = glob.expand(obj.shape[0], 256, 4, 4)
        x = torch.cat((obj, glob2), dim=1) # (N, 2048+256, 4, 4)
...
class Selector(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(2048+256, 256, 2)
        self.relu1 = nn.ReLU(inplace=True)
        self.conv2 = nn.Conv2d(256, 16, 2)
        self.relu2 = nn.ReLU(inplace=True)
        self.conv3 = nn.Conv2d(16, 1, 2)

    def forward(self, x):
        # x.shape (N, 2048+256, 4, 4)
        weights = self.relu1(self.conv1(x)) # (N, 256, 3, 3)

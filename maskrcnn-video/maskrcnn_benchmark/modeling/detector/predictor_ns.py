...
class Predictor(nn.Module):
    ...
    def __init__(self, cfg=None, select_num=10, class_num=4):
    ...
    def forward(self, glob_features, roi_features): #(1, 1024, 40, 23),(n,2048,7,7)
        # Processing global feature
        glob = self.avgpool_glob1(glob_features) # (1, 1024, 14, 14)
        glob = self.relu_glob1(self.conv_glob1(glob)) # (1, 512, 14, 14)
        glob = self.relu_glob2(self.conv_glob2(glob)) # (1, 256, 14, 14)
        glob = self.avgpool_glob2(glob) # (1, 256, 1, 1)
        # Processing object features
        # obj = roi_features # (N, 2048, 7, 7)
        ...
        x = torch.squeeze(glob)
        x = self.drop(self.relu4(self.fc4(x)))
...
if __name__ == '__main__':
    ...
    roi = torch.randn((20,2048,7,7))
    glob = torch.randn((1,1024,40,23))
    roi = roi.to(device)
    glob = glob.to(device)
    # x = {'roi_features':roi,
    #      'glob_feature':glob}

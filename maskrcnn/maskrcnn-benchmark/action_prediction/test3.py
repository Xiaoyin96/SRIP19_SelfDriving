...
from baseline3 import baseline
...
#    imBatch = imBatch.cuda(device)
    ...
    for i, dataBatch in enumerate(dataloader):
        ...
        if target_cpu.data.numpy()[0] == 0:
            count0 += 1
            if action == 0:
                Accuracy_class[0] += 1
                accuracy += 1
        elif target_cpu.data.numpy()[0] == 1:
            count1 += 1
            if action == 1:
                Accuracy_class[1] += 1
                accuracy += 1
    ...
    print('Class Forward accuracy:', Accuracy_class[0]/count0)
    print('Class Stop accuracy:', Accuracy_class[1]/count1)
